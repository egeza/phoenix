...
include { ABRICATE as ABRICATE_ECOLI_VF  } from '../modules/local/abricate'
include { ABRICATE as ABRICATE_PF        } from '../modules/local/abricate'
...
workflow PHOENIX_EXTERNAL {
    ...
    main:
        ...
        GET_TRIMD_STATS (
            passed_fastp_json_ch, false, workflow.manifest.version // false says no busco is being run
        )
        ...
        // --- Abricate: virulence genes for ALL samples (vfdb default; ecoli_vf for Escherichia) ---
        scaffolds_and_taxa_ab_ch = BBMAP_REFORMAT.out.filtered_scaffolds
            .join(DETERMINE_TAXA_ID.out.taxonomy.map{ meta, tax -> [[id:meta.id], tax] }, by: [0])

        ecoli_vf_scaffolds_ch = scaffolds_and_taxa_ab_ch
            .filter { meta, scaffolds, tax -> new File(tax.toString()).text.contains("Escherichia") }
            .map { meta, scaffolds, tax -> [meta, scaffolds] }

        non_ecoli_scaffolds_ch = scaffolds_and_taxa_ab_ch
            .filter { meta, scaffolds, tax -> !new File(tax.toString()).text.contains("Escherichia") }
            .map { meta, scaffolds, tax -> [meta, scaffolds] }

        ABRICATE_ECOLI_VF(ecoli_vf_scaffolds_ch.map{ meta, scaffolds -> [meta, scaffolds, "ecoli_vf"] })
        ch_versions = ch_versions.mix(ABRICATE_ECOLI_VF.out.versions)
        ...
        // --- Abricate: plasmid detection for ALL samples (plasmidfinder) ---
        ABRICATE_PF(BBMAP_REFORMAT.out.filtered_scaffolds.map{ meta, scaffolds -> [meta, scaffolds, "plasmidfinder"] })
        ch_versions = ch_versions.mix(ABRICATE_PF.out.versions)
        abricate_plasmidfinder_ch = ABRICATE_PF.out.report
        ...
        GATHER_SUMMARY_LINES (
            [], all_summaries_ch, outdir_path, false, workflow.manifest.version
        )
