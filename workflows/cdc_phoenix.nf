...
workflow PHOENIX_EXQC {
    ...
    main:
        ...
        centar_boolean = DETERMINE_TAXA_ID.out.taxonomy.map{ it -> get_only_taxa(it) }.collect().flatten().count{ it -> it == "Clostridioides"}.map{ it -> it > 0 }
        ...
        GRIPHIN (
            ...
            params.coverage, true, shigapass_var, centar_var, params.bldb, false, false, [], "", "" //Add empty string to show there is no old_version_info and another set to show it did not try to infer mode (only useful for update mode)
        )
    ...
    emit:
        ...
        phx_summary      = GATHER_SUMMARY_LINES.out.summary_report  // Phoenix_output_report
